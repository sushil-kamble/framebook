...
describe('framebook service', () => {
  ...
  it('creates, lists, updates, and archives topics', async () => {
    const topic = await service.createTopic({
      name: 'Travel Poster Study',
      description: 'Swiss Alps poster experiments',
      instruction: 'Use cinematic travel poster style with bold typography space.',
  ...
  it('stores metadata in a local sqlite database inside the workspace', async () => {
    ...
    await expect(access(path.join(dataDir, 'framebook.db'))).resolves.toBeUndefined()
  ...
  it('uses codex app-server client when enhancing prompts', async () => {
    const codexEnhancedPrompt = 'Photorealistic morning tea stall prompt with clear constraints.'
...
async function createTopic(service) {
  return service.createTopic({
    ...
    basePromptDetails: 'Two travelers, one small backpack, recurring red scooter, misty hills',
    enhancerMode: 'storyboard',
  })
}
